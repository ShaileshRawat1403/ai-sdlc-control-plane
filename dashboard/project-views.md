# Project Views

<!-- brainbench:generated:start -->

## Systems Registry Overview

| System | Role | Repository | Status | Priority |
|---|---|---|---|---|
| **BrainBench** | Operating memory and validation control plane | `ShaileshRawat1403/ai-sdlc-control-plane` | `active` | `high` |
| **DAX** | Governed execution workstation for AI-assisted SDLC | `ShaileshRawat1403/dax` | `active` | `high` |
| **Rook** | Repo/operator workbench for visible, governable AI-assisted work | `ShaileshRawat1403/rook` | `active` | `high` |
| **Soothsayer** | Multi-user governance workspace catalog and approvals inbox | `ShaileshRawat1403/soothsayer` | `paused` | `medium` |
| **Flowright** | Governance-first workflow execution kernel and orchestration | `ShaileshRawat1403/flowright` | `paused` | `medium` |
| **ToolSmith** | Workflow utilities, repo-specific helpers, and repeatable conversion tools | `ShaileshRawat1403/ToolSmith` | `paused` | `medium` |
| **Tessera** | Reusable SDK/job-pack patterns for evals, repo tools, prompts, RAG, and transformations | `none` | `paused` | `medium` |
| **Picobot** | Lightweight ingress and channel bridge | `none` | `unmapped` | `medium` |
| **PruningMyPothos** | Website, documentation, positioning, and content surface | `none` | `unmapped` | `medium` |

<!-- brainbench:generated:end -->

## Human Notes
[Add manual notes here. These will be preserved by refresh script.]
